<script lang="ts">
	...
	import { COMMON_IMAGES, createImageWithCaption, isEmptyImage } from '../../helpers/images';
	import { addUserImage, removeUserImage, userImages } from '../../services/user_images.svelte';
	...
	const onSave = () => {
		...
		const image = createImageWithCaption(imageUnderConstruction, scopedT('ownImageCaption'));
...
<footer aria-label={scopedT('savedImages')}>
	...
	<div class="saved-images" role="list" aria-label={scopedT('savedImagesAriaLabel')}>
		...
		{#each userImages as image (image)}
			<button
