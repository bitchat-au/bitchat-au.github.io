<script lang="ts">
	...
	import { cloneImage, COMMON_IMAGES, type ImageMatrix } from '../../helpers/images';
	...
	// Using a writeable derive like the following rule suggests, doesn't work because the derived value is not deeply reactive, so we need to use a state instead.
...
<Dialog open {onClose} ariaLabel={scopedT('dialogAriaLabel')} class="hacker-modal">
	...
	<div class="header">
		<InteractiveBitString bind:image={modifiedMessage} onHover={(index) => (hoverIndex = index)} />
	</div>
	...
	<section>
		<ImageMatrixRenderer
			...
			highlightedPixel={hoverIndex !== null ? [Math.floor(hoverIndex / 5), hoverIndex % 5] : null}
		/>
	</section>
	...
	<footer>
		<button class="transparent" onclick={onClose}>{scopedT('cancel')}</button>
		<button onclick={() => onResult({ newMessage: modifiedMessage })}>{scopedT('modify')}</button>
	</footer>
</Dialog>
